...
$query = $pdo->query("SELECT * FROM journal WHERE statut = 4 ORDER BY id DESC LIMIT $start_from,$record_per_page");
...
$page_query = $pdo->query('SELECT * FROM journal WHERE statut = 4 ORDER BY id DESC');
...
if ($page > 1) {
  $previous = $page - 1;
  $result['resultat'] .= '<li class="pagination_link_error" id="' . $previous . '"><span class="page-link"><i class="fas fa-caret-left"></i> Précédent</span></li>';
}
...
if ($page < $total_pages) {
  $page++;
  $result['resultat'] .= '<li class="pagination_link_error" id="' . $page . '"><span class="page-link">Suivant <i class="fas fa-caret-right"></i></span></li>';
}
$result['resultat'] .= '</ul>';
$result['resultat'] .= '</div>';
