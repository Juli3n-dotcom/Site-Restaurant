...
Modale d'Update d'une Sous categories a partir  DrinksSub.php en Ajax
...
if (isset($_POST['cat_id'])) {
  ...
  while ($row = $query->fetch()) {
    ...
    if ($options['show_sub_cat_drink_photo']) {
      ...
      $result .= DrinksSubCategories::getCatPhoto($pdo, $row['photo_id'], $ua['name']);
    ...
    $result .=         ' <option value="' . $row['cat_id'] . '" disabled selected>' . DrinksSubCategories::getParent($pdo, $row['cat_id']) . '</option>';
