...
$output .= '<th class="tab desktop">Catégorie Parente</th>';
if ($options['show_cat_drink_description'] == 1) {
  $output .= '<th class="desktop">Description</th>';
}
$output .= '<th class="desktop"># Boissons</th>';
...
while ($row = $query->fetch()) {
  ...
  if ($options['show_sub_cat_drink_photo'] == 1) {
    ...
    $output .= '<td class="td-team">' . DrinksSubCategories::getImage($pdo, $row['photo_id'], $ua['name']) . '</td>';
  }
  $output .= '<td class="tab desktop"><strong>' . DrinksSubCategories::getParent($pdo, $row['cat_id']) . '</strong></td>';
  if ($options['show_cat_drink_description'] == 1) {
    $output .= '<td class="desktop">' . substr($row['description'], 0, 45) .  '</td>';
  }
  $output .= '<td class="desktop">' . DrinksSubCategories::getNbDrinks($pdo, $row['id']) . '</td>';
  if ($row['est_publie'] == 1) {
