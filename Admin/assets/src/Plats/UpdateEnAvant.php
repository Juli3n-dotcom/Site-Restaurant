...
if (!empty($_POST)) {
  ...
  if ($est_en_avant == 0) {
    ...
    $req = $pdo->prepare('UPDATE plats SET est_en_avant = :en_avant WHERE id = :id');
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'Plat mit en avant');
    postJournal($pdo, $user, 1, 'plat mit en avant', 'plat mit en avant # ' . $id);
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'Le plat n\'est plus mit en avant');
    postJournal($pdo, $user, 1, 'mit en avant', 'Le plat #' . $id . ' n\'est plus mit en avant');
    ...
    while ($row = $query->fetch()) {
      ...
      $result['resultat'] .= '<td class="member_action">';
      $result['resultat'] .= '<div class="member_action-container">';
      $result['resultat'] .= '<a href="" class="linkbtn"><i class="fa-regular fa-eye"></i></a>';
      $result['resultat'] .= '<input type="button" class="viewbtn" name="view" id="' . $row['id'] . '"></input>';
      $result['resultat'] .= '<input type="button" class="editbtn" id="' . $row['id'] . '"></input>';
      $result['resultat'] .= '<input type="button" class="deletebtn"></input>';
      $result['resultat'] .= '</div>';
