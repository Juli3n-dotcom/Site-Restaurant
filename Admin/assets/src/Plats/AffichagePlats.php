...
while ($row = $query->fetch()) {
  ...
  if ($options['show_plat_photo'] == 1) {
    ...
    $output .= '<td class="td-team">' . Plats::getImage($pdo, $row['photo_id'], $ua['name']) . '</td>';
  }
  $output .= '<td class="desktop">' . $row['prix'] . ' €</td>';
  $output .= '<td class="desktop description">' . $row['description'] . '</td>';
  $output .= '<td class="tab desktop"><strong>' . Plats::getCat($pdo, $row['cat_id']) . '</strong></td>';
  if ($options['show_plat_stats'] == 1) {
    $output .= '<td class="desktop">0</td>';
  }
  if ($options['show_plat_en_avant'] == 1) {
    $output .= '<td class="tab desktop">' . Plats::getEstEnAvant($pdo, $row['id']) . '</td>';
  }
  if ($row['est_publie'] == 1) {
  ...
  $output .= '<a href="FichePlat.php?id=' . $row['id'] . '" class="linkbtn"><i class="fa-regular fa-eye"></i></a>';
