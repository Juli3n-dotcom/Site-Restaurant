...
Update de la publication d'un plat a partir Plats.php en Ajax
...
if (!empty($_POST)) {
  ...
  if ($publie == 0) {
    ...
    $req = $pdo->prepare('UPDATE plats SET est_publie = :publie WHERE id = :id');
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'Plat Affiché');
    postJournal($pdo, $user, 1, 'plat Affiché', 'plat Affiché # ' . $id);
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'plat Retiré');
    postJournal($pdo, $user, 1, 'plat Retiré', 'plat Retiré # ' . $id);
