...
Update de la position d'une sous categories a partir categorie.php en Ajax
...
if (!empty($_POST['data'])) {
  ...
  foreach ($obj as $cat) {
    ...
    $updatePos = $pdo->prepare('UPDATE plats_sous_categories SET position = :position WHERE id=' . $id);
  ...
  #3 retour ajax
  $result['status'] = true;
  $result['notif'] = Notifications::notif('success', 'Organisation des sous catégories modifiée');
