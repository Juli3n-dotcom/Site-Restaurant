...
while ($row = $query->fetch()) {
  ...
  if ($options['show_cat_photo'] == 1) {
    ...
    $output .= '<td class="td-team">' . SousCategories::getImage($pdo, $row['photo_id'], $ua['name']) . '</td>';
  }
  $output .= '<td class="tab desktop"><strong>' . SousCategories::getParent($pdo, $row['cat_id']) . '</strong></td>';
  if ($options['show_cat_description'] == 1) {
