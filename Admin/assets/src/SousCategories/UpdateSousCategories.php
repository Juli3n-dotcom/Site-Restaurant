...
Update d'une sous categories a partir SousCat.php en Ajax
...
if (!empty($_POST)) {
  ...
  $param = FALSE;
  $requete = 'UPDATE plats_sous_categories SET ';

  if (empty($titre)) {
    $result['status'] = false;
    $result['notif'] = Notifications::notif('warning', 'oups! il manque le titre');
    postJournal($pdo, $user, 5, 'Tentative de modification d\'une sous catégorie de plats', 'Titre manquant');
  } else {
    ...
    if ($titre !== $thisCat['titre']) {
      if (!preg_match('~^[a-zàâçéèêëîïôûùüÿñæœA-Z- ]+$~', $titre)) {
        ...
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une sous catégorie de plats', 'Caractére non autorisé');
      } else if (getCatBy($pdo, 'titre', $titre) !== null) {

        $result['status'] = false;
        $result['notif'] = Notifications::notif('warning', 'oups cette catégorie existe déjà');
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une sous catégorie de plats', 'Catégorie déjà existante');
      } else {
        $requete .= ' titre = :titre';
  ...
  if ($options['show_cat_photo']) {
    ...
    if (!empty($_FILES['new_img']['tmp_name'])) {
      ...
      if ($_FILES['new_img']['error'] !== UPLOAD_ERR_OK) {
        ...
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une sous catégorie de plats', 'Probléme lors de l\'envoi du fichier.code ' . $_FILES['new_img']['error']);
      } elseif ($_FILES['new_img']['size'] < 12) {

        $result['status'] = false;
        $result['notif'] = Notifications::notif('error', 'Le fichier envoyé n\'est pas une image');
        postJournal($pdo, $user, 4, 'Tentative de modification d\'une sous catégorie de plats', 'Le fichier envoyé n\'est pas une image');
      } elseif (!in_array($extension, $allowTypes)) {
        $result['status'] = false;
        $result['notif'] = Notifications::notif('warning', 'Format d\'image non pris en charge');
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une sous catégorie de plats', 'Format d\'image non pris en charge');
      } else {
        ...
        if (!move_uploaded_file($_FILES['new_img']['tmp_name'], $complete_path)) {
          ...
          postJournal($pdo, $user, 4, 'Tentative de modification d\'une sous catégorie de plats', 'La photo n\'a pas pu être enregistrée');
        } else {
  ...
  $result['status'] = true;
  $result['notif'] = Notifications::notif('success', 'Sous Catégorie modifiée');
