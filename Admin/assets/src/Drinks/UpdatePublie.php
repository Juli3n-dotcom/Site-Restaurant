...
Update de la publication d'une boisson a partir Drinks.php en Ajax
...
if (!empty($_POST)) {
  ...
  if ($publie == 0) {
    ...
    $req = $pdo->prepare('UPDATE boissons SET est_publie = :publie WHERE id = :id');
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'Boisson Affichée');
    postJournal($pdo, $user, 1, 'Boisson Affichée', 'Boisson Affichée # ' . $id);
    ...
    $result['status'] = true;
    $result['notif'] = General::notif('success', 'Boissons Retirée');
    postJournal($pdo, $user, 1, 'Boissons Retirée', 'Boissons Retirée # ' . $id);
