...
Update d'une categories a partir  DrinksCat.php en Ajax
...
if (!empty($_POST)) {
  ...
  $data = $pdo->query("SELECT * FROM boissons_categories WHERE id = '$id'");
  $thisCat = $data->fetch(PDO::FETCH_ASSOC);

  // debut de la requete d'update
  $param = FALSE;
  $requete = 'UPDATE boissons_categories SET ';

  if (empty($titre)) {
    $result['status'] = false;
    $result['notif'] = General::notif('warning', 'oups! il manque le titre');
    postJournal($pdo, $user, 5, 'Tentative de modification d\'une catégorie de boissons', 'Titre manquant');
  } else {
    ...
    if ($titre !== $thisCat['titre']) {
      if (!preg_match('~^[a-zàâçéèêëîïôûùüÿñæœA-Z- ]+$~', $titre)) {
        ...
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une catégorie de boissons', 'Caractére non autorisé');
      } else if (getCatBy($pdo, 'titre', $titre) !== null) {

        $result['status'] = false;
        $result['notif'] = General::notif('warning', 'oups cette catégorie existe déjà');
        postJournal($pdo, $user, 5, 'Tentative de modification d\'une catégorie de boissons', 'Catégorie déjà existante');
      } else {
        $requete .= ' titre = :titre';
