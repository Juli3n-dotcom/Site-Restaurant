...
Delete d'une catégorie de plats à partir DrinksCat.php en Ajax
...
if (!empty($_POST)) {
  ...
  if (($_POST['confirmedelete']) !== $confirme) {
    ...
    #3
    $req = $pdo->query("SELECT id, position FROM boissons_categories WHERE position > $position");
    $upCat = $req->fetchAll(PDO::FETCH_ASSOC);

    #4
    foreach ($upCat as $cats) {
      $updatePos = $pdo->prepare('UPDATE boissons_categories SET position = :position WHERE id=' . $cats['id']);
    ...
    #6
    $req2 = $pdo->exec("DELETE FROM boissons_categories WHERE id = '$id'");
