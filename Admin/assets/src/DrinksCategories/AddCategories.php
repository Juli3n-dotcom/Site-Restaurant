...
Ajout d'une catégorie de plats à partir DrinksCat.php en Ajax
...
if (!empty($_POST)) {
  ...
  if (empty($titre) && !preg_match('~^[a-zA-Z- ]+$~', $titre)) {
    ...
    postJournal($pdo, $user, 5, 'Tentative d\'ajout d\'une catégorie de boisson', 'Titre manquant');
  } else if (getCatBy($pdo, 'titre', $titre) !== null) {

    $result['status'] = false;
    $result['notif'] = General::notif('warning', 'oups cette catégorie existe déjà');
    postJournal($pdo, $user, 5, 'Tentative d\'ajout d\'une catégorie de boisson', 'Catégorie déjà existante');
  } else if ($options['show_cat_drink_photo']) {
    if (empty($_FILES['add_img']['name'])) {

      $result['status'] = false;
      $result['notif'] = General::notif('warning', 'oups il manque la photo de catégorie');
      postJournal($pdo, $user, 5, 'Tentative d\'ajout d\'une catégorie de boisons', 'oups il manque la photo de catégorie');
    }
  } else {
    ...
    if ($options['show_cat_drink_photo']) {
      ...
      if ($_FILES['add_img']['error'] !== UPLOAD_ERR_OK) {
        ...
        postJournal($pdo, $user, 5, 'Tentative d\'ajout d\'une catégorie de boisson', 'Probléme lors de l\'envoi du fichier.code ' . $_FILES['add_img']['error']);
      } elseif ($_FILES['add_img']['size'] < 12) {

        $result['status'] = false;
        $result['notif'] = General::notif('error', 'Le fichier envoyé n\'est pas une image');
        postJournal($pdo, $user, 4, 'Tentative d\'ajout d\'une catégorie de boisson', 'Le fichier envoyé n\'est pas une image');
      } elseif (!in_array($extension, $allowTypes)) {
        $result['status'] = false;
        $result['notif'] = General::notif('warning', 'Format d\'image non pris en charge');
        postJournal($pdo, $user, 5, 'Tentative d\'ajout d\'une catégorie de boisson', 'Format d\'image non pris en charge');
      } else {
        ...
        if (!move_uploaded_file($_FILES['add_img']['tmp_name'], $complete_path)) {
          ...
          postJournal($pdo, $user, 4, 'Tentative d\'ajout d\'une catégorie de boisson', 'La photo n\'a pas pu être enregistrée');
        } else {
    ...
    $req2 = $pdo->prepare('INSERT INTO boissons_categories(titre,
                                                        description, 
                                                        photo_id,
                                                        author_id,
                                                        position,
